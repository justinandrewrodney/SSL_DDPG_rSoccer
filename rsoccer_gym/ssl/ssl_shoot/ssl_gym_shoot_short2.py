...
class SSLShootShort2Env(SSLBaseEnv):
    ...
    def __angle_goal_rwold(self):
        if(not self.kicked):
            return 0
        if(not  self.just_kicked ):
            return 0
        ...
        angle_diff_larger = max(abs(angle_diff_bot), abs(angle_diff_top))
        if(angle_diff_larger <= angle_for_goal): #if robot facing between goal posts
            print("REward towards: " +str(4*(angle_for_goal - angle_diff_larger)/(angle_for_goal/2)*ball_vel))
            #return math.exp(.5*(angle_for_goal - angle_diff_larger)/angle_for_goal + ball_vel )
            return 4*(angle_for_goal - angle_diff_larger)/(angle_for_goal/2)*ball_vel
        else: 
            #if robot facing away from goal posts penalize(value will be negative)
